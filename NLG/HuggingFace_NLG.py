...
class NLG_API:
    def __init__(self, huggingface_api_key):
        ...
        self.api_url = "https://api-inference.huggingface.co/models/Qwen/Qwen2.5-Coder-32B-Instruct" # Qwen/Qwen2.5-Coder-32B-Instruct, meta-llama/Llama-3.1-8B-Instruct
    ...
    def query_model(self, prompt):
        ...
        try:
            ...
            if isinstance(response_json, list) and len(response_json) > 0:
                return response_json[0].get('generated_text', '').replace(full_prompt, '').strip()
            return ''
    ...
    def detect_intent(self, user_input):
        ...
        intent_pair = response.strip().split(", ")
    ...
    def generate_slot_response(self, previous_question, user_input, intent, slot, ir_examples, guided_mode=False, guided_mode_examples=None):
        ...
        if guided_mode == False:
            ...
            """
            question_response = self.query_model(question_prompt)
            ...
            """
            relevance_response = self.query_model(relevance_prompt)
            ...
            final_response = f"guided mode: {relevance_response}\n\n{question_response}"
            return final_response, fa_slot
    ...
    def update_dict_based_on_input(self, input_dict, user_input):
        ...
        try:
            response = self.query_model(prompt)
            print(f'update_dict_based_on_input response: {response}')
